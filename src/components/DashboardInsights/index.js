...
const DashboardInsights = ({ budgetCycle, budgetCycleTransactions, budgetsData })=>{
  ...
  const runInsights = ()=>{
    ...
    //Declare variables
    const budgetsDataWithSpent = budgetsData.map(b=>({
      ...b,
    ...
    const incomeExpected = budgetsData.find(budgetData=>budgetData.Name==="Infor payroll").Amount;
    const incomeBudgetsData = budgetsData.filter(budgetData=>budgetData.Type==="income");
    const incomeEarned = incomeBudgetsData.reduce((total,b)=>total+=getBudgetAmountSpentFromTransactions(b.Name, budgetCycleTransactions.all), 0);
    ...
    const savingsBudgetData = budgetsData.find(budgetData=>budgetData.Name==="Savings");
